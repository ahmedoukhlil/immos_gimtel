                                ...
                                <div class="flex flex-wrap items-center gap-1.5">
                                    ...
                                    @if($isAdmin)
                                        <div x-data="{ open: false }" class="relative">
                                            <button @click="open = !open" class="p-1 rounded text-gray-400 hover:text-indigo-600 hover:bg-indigo-50 transition-colors" title="Gérer les agents">
                                                <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6v6m0 0v6m0-6h6m-6 0H6"/></svg>
                                            </button>
                                            <div x-show="open" @click.away="open = false" x-transition x-cloak class="absolute right-0 mt-1 w-52 bg-white rounded-lg shadow-lg border border-gray-200 py-1 z-20 max-h-60 overflow-y-auto">
                                                <p class="px-3 py-1.5 text-xs font-semibold text-gray-400 uppercase">Agents</p>
                                                @foreach($this->agents as $agent)
                                                    @php $isAssigned = $allAgents->contains('idUser', $agent->idUser); @endphp
                                                    <button wire:click="toggleAgentLocalisation({{ $invLoc->id }}, {{ $agent->idUser }})" @click="open = false" class="w-full flex items-center gap-2 text-left px-3 py-2 text-sm hover:bg-indigo-50 {{ $isAssigned ? 'bg-indigo-50/60' : 'text-gray-700' }}">
                                                        <div class="w-4 h-4 rounded border-2 flex items-center justify-center flex-shrink-0 {{ $isAssigned ? 'bg-indigo-600 border-indigo-600' : 'border-gray-300 bg-white' }}">
                                                            @if($isAssigned)
                                                        ...
                                                        <span class="{{ $isAssigned ? 'text-indigo-700 font-medium' : '' }}">{{ $agent->users ?? 'Agent' }}</span>
                                                    </button>
                                                @endforeach
                                            </div>
                                        </div>
